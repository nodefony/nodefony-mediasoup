...
<v-card class="d-flex flex-wrap" :max-width="maxWidth" :max-height="maxHeight" :min-width="minWidth" :min-height="minHeight">
  ...
  <v-expand-transition>
    <div v-show="!isMinimize">
      <v-container v-show="mdPeer" style="padding:0px;width:100%">
        <div class="media-content">
          <video v-show="this.video" muted playsinline :controls="false" ref="video">
          </video>
          <div v-show="!this.video" class="pa-4 text-center">
          ...
          <v-btn v-show="mdRoom" small :disabled="false" color="blue-grey" class="ma-2 white--text buttons" fab>
            <div v-if="this.audio">
      ...
      <v-card-actions>
        <v-row class="fill-height" align-content="center" justify="center">
          <v-col cols="6">
            <v-btn v-show="mdRoom  && isMe" small :loading="loadingVideo" :disabled="loadingVideo" color="blue-grey" class="ma-2 white--text" fab @click="videoButton">
              <v-icon v-if="this.video">mdi-video-box</v-icon>
              <v-icon v-else dark>mdi-video-box-off</v-icon>
            </v-btn>
            <v-btn v-show="mdRoom && isMe" small :loading="loadingAudio" :disabled="loadingAudio" color="blue-grey" class="ma-2 white--text" fab @click="audioButton">
              <v-icon v-if="this.audio">mdi-volume-high</v-icon>
              <v-icon v-else dark>mdi-volume-off</v-icon>
            </v-btn>
            <v-btn v-show="mdRoom  && isMe" small :loading="loadingMonitor" :disabled="loadingMonitor" color="blue-grey" class="ma-2 white--text" fab @click="monitorButton">
              <v-icon v-if="screen">mdi-monitor-share</v-icon>
              <v-icon v-else dark>mdi-monitor-off</v-icon>
            </v-btn>
            <v-spacer></v-spacer>
            <v-btn v-show="mdRoom  && !isMe" icon @click="expendMenu">
  ...
  <v-expand-transition>
    <div v-show="isMinimize" style="padding:0px;width:100%">
...
<script>
import Message from "@/components/mediasoup/Message";
export default {
  name: 'Media',
  components: {
    Message
  ...
  mounted() {
    if (this.isMe) {
      this.$mediasoup.on("connect", (room, peer) => {
        this.mdRoom = room;
        this.mdPeer = peer;
        ...
        case "audio":
          {
            ...
            return tag
              ...
              .then(() => {
                this.demuteTag();
                this.audio = true;
                this.log(`Play audio tag ${this.name}`);
              })
              .catch((error) => {
                this.muteTag();
