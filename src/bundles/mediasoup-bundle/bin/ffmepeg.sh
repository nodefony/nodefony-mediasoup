...
#
eval "$(echo ${res} | jq -r '@sh "audioTransportId=\(.result.transport.id) audioTransportIp=\(.result.transport.ip) audioTransportPort=\(.result.transport.port) audioTransportRtcpPort=\(.result.transport.rtcpPort)"')"
...
# IP, port and RTCP port.
#
eval "$(echo ${res} | jq -r '@sh "videoTransportId=\(.result.transport.id) videoTransportIp=\(.result.transport.ip) videoTransportPort=\(.result.transport.port) videoTransportRtcpPort=\(.result.transport.rtcpPort)"')"
