...
touch $ROOT_CA/db/nodefony-mediasoup-ca.db
touch $ROOT_CA/db/nodefony-mediasoup-ca.db.attr
...
touch $ROOT_CA_INTERMEDIATE/db/nodefony-mediasoup-ca.db
touch $ROOT_CA_INTERMEDIATE/db/nodefony-mediasoup-ca.db.attr
...
#  Create the root key
#Encrypt the root key with AES 256-bit encryption and a strong password.
#openssl genrsa -aes256  -out $ROOT_CA_INTERMEDIATE/private/nodefony-mediasoup.key.pem 2048
openssl genrsa \
  -out $ROOT_CA_INTERMEDIATE/private/nodefony-mediasoup.key.pem \
  2048
# chmod 400 $ROOT_CA_INTERMEDIATE/private/nodefony-mediasoup.key.pem


openssl req -config $CONF_DIR_INTERMEDIATE/openssl.cnf \
  -key $ROOT_CA_INTERMEDIATE/private/nodefony-mediasoup.key.pem \
  -new -sha256 -out $ROOT_CA_INTERMEDIATE/csr/nodefony-mediasoup.csr.pem

#Create  server a certificate
# sign the CSR
openssl ca -config $CONF_DIR_INTERMEDIATE/openssl.cnf -batch \
  -extensions server_cert -days 375 -notext -md sha256 \
  -in $ROOT_CA_INTERMEDIATE/csr/nodefony-mediasoup.csr.pem \
  -out $ROOT_CA_INTERMEDIATE/certs/server.nodefony-mediasoup.cert.pem
# chmod 444 $ROOT_CA_INTERMEDIATE/certs/server.nodefony-mediasoup.cert.pem

openssl x509 -noout -text \
  -in $ROOT_CA_INTERMEDIATE/certs/server.nodefony-mediasoup.cert.pem
...
openssl ca -config $CONF_DIR_INTERMEDIATE/openssl.cnf -batch \
  ...
  -in $ROOT_CA_INTERMEDIATE/csr/nodefony-mediasoup.csr.pem  \
  -out $ROOT_CA_INTERMEDIATE/certs/client.nodefony-mediasoup.cert.pem
# chmod 444 $ROOT_CA_INTERMEDIATE/certs/client.nodefony-mediasoup.cert.pem
...
# create fullchain
cat $ROOT_CA_INTERMEDIATE/certs/server.nodefony-mediasoup.cert.pem \
...
cp -r $ROOT_CA/certs/ca.cert.pem $ROOT_DIR/ca/nodefony-mediasoup-root-ca.crt.pem
cp -r $ROOT_CA/private/ca.key.pem $ROOT_DIR/ca/nodefony-mediasoup-root-ca.key.pem
...
cp -r $ROOT_CA_INTERMEDIATE/private/nodefony-mediasoup.key.pem $ROOT_DIR/server/privkey.pem
cp -r $ROOT_CA_INTERMEDIATE/certs/server.nodefony-mediasoup.cert.pem $ROOT_DIR/server/cert.pem
...
openssl verify -CAfile $ROOT_CA_INTERMEDIATE/certs/ca-chain.cert.pem \
  $ROOT_CA_INTERMEDIATE/certs/server.nodefony-mediasoup.cert.pem

# verify server
openssl verify -CAfile $ROOT_CA_INTERMEDIATE/certs/ca-chain.cert.pem \
  $ROOT_CA_INTERMEDIATE/certs/client.nodefony-mediasoup.cert.pem

openssl rsa -noout -modulus -in $ROOT_DIR/server/privkey.pem
openssl req -noout -modulus -in $ROOT_CA_INTERMEDIATE/csr/nodefony-mediasoup.csr.pem
